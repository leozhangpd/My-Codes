...
//btn1.onclick = q=>console.log(q);
//fun = e => console.log(e);
//function fun(e){console.log();};
...
var arrS = (function rtn() {//打乱顺序
    ...
    sessionStorage.setItem("data",JSON.stringify(arr));//通过JSON.stringify，把arr包装成字符串（只在数组的外围添加双引号，不修改内容结构）赋给sessionStorage，这样就不用后者再进行转换成字符串了，否则直接让sessionStorage把数组转换的话，数组arr的结构会被破坏，到时候还得split方法将字符串转化成数组
    //console.log(arr);
    return rtn;    
})();
...
/* $('#btn2').click((e) =>{
    ...
    } else{window.location.href = "show_id.html"}
    }); */
